...
if [[ "$1" == "rofi" ]]; then
    ...
    if [ ! -z $choice ] ;then
        echo "hyprshade_filter=\"$choice\"" > ~/.config/imkool/settings/hyprshade.sh
        if [ "$choice" == "off" ] ;then
            hyprshade off
    ...
    if [ -f ~/.config/imkool/settings/hyprshade.sh ] ;then
        source ~/.config/imkool/settings/hyprshade.sh
    fi
