...
while true; do
	...
	# Show ROFI menu
	chosen=$(printf "%b" "$menu_list" | rofi -theme-str 'configuration { show-icons: false; } window { location: north east; x-offset: -260; y-offset: 155; width: 225px; } listview { columns: 1; lines: 5; } inputbar { enabled: false; } mode-switcher { enabled: false; }' -dmenu -i)
	[ -z "$chosen" ] && break
	...
	*)
		...
		if echo "$saved_connections" | grep -Fxq "$ssid"; then
			...
			if [ "$sec" != "--" ] && [ -n "$sec" ]; then
				while :; do
					pass=$(rofi -dmenu -theme-str 'window { location: north east; x-offset: -260; y-offset: 155; width: 225px; } listview { enabled: false; } mode-switcher { enabled: false; } element { enabled: false; }' -dmenu -mesg "Password for $ssid:" -password)
